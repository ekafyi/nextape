...
class IndexPage extends Component {
    ...
    if (this.props.router.query.artist && this.props.accessToken) {
      ...
      spotifyApi
        ...
        .then(function(artistTracksData) {
          ...
          const artistTrack = getRandomItemInArray(
            artistTracksData.tracks,
            tracksLength
          );
          // save to states
          currentComponent.setState({
        ...
        .then(function(relatedArtistsData) {
          ...
          const relArtLength =
            relatedArtistsData.artists.length - howManyRelArt - 1;
          const randomNum = Math.floor(
            Math.random() * Math.floor(relArtLength)
          );
          ...
          for (var i = randomNum; i < randomNum + howManyRelArt; i++) {
            ...
            spotifyApi
              .searchTracks(
                'artist:"' + relatedArtistsData.artists[i].name + '"'
              )
              .then(
                function(trackResponse) {
                  ...
                  let relArtTrack = getRandomItemInArray(
  ...
  render() {
    const { artist, artistTrack, relatedTracks, loading } = this.state;

    // console.log(this.props.router.query)

    return (
      <Layout>
        {this.props.router.query.artist ? (
          <React.Fragment>
            {loading ? (
              <div>Loading...</div>
            ) : (
          ...
          <div>
            <p>please enter your favourite artist (in the URL for now, sorry! 🙈)</p>
            <p>example:<br/>
              http://localhost:3000?artist=pavement<br/>
              http://localhost:3000?artist=stone-roses
            </p>
          </div>
        )}
      </Layout>
